...
func NewPluginRegistry(ctx context.Context) *PluginRegistry {
	pr := PluginRegistry{
		Context: ctx,
	}
...
func (r *PluginRegistry) RegisterTargetManager(pluginName string, tmf target.TargetManagerFactory) error {
	...
	r.lock.Lock()
	defer r.lock.Unlock()
	logging.Infof(r.Context, "Registering target manager %s", pluginName)
...
func (r *PluginRegistry) RegisterTestFetcher(pluginName string, tff test.TestFetcherFactory) error {
	...
	r.lock.Lock()
	defer r.lock.Unlock()
	logging.Infof(r.Context, "Registering test fetcher %s", pluginName)
...
func (r *PluginRegistry) RegisterTestStep(pluginName string, tsf test.TestStepFactory, stepEvents []event.Name) error {
	...
	r.lock.Lock()
	defer r.lock.Unlock()
	logging.Infof(r.Context, "Registering test step %s", pluginName)
...
func (r *PluginRegistry) RegisterReporter(pluginName string, rf job.ReporterFactory) error {
	...
	r.lock.Lock()
	defer r.lock.Unlock()
	logging.Infof(r.Context, "Registering reporter %s", pluginName)
